...
class HeaderTests {
  ...
  fun testRequestHeaders() {
    runBlocking {
      ...
      //assertEquals("close", headers[Headers.CONNECTION]?.toLowerCase())
      assertEquals("httpbin.org", headers[Headers.HOST])
      assertEquals("asynk/0.0.0.18", headers[Headers.USER_AGENT])
  ...
  fun testResponseHeaders() {
    runBlocking {
      ...
      assertEquals("asynk/0.0.0.18", entry.value)
    }
  }
